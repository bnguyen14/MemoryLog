...
class MemoryLog {
	...
	//Declare important parts of the program.
	Scanner scan;

	//Declare ArrayList that will manage reading and writing of events.
	...
	public MemoryLog() {
		...
		itemList = new File("C:\\Users\\Dylan\\Desktop\\my_files\\my_utilities\\memory_log_v2\\memorylog\\auto_memory_log.txt");
		date = LocalDate.now();
		loadEntries();
	}
	...
	public void loadEntries() {
		...
		//Only continue if the file could be found.
	...
	public void processIndex(Item passedItem, int addToAddThis) {
		
		//User needs to make sure that the addThis is changed immediately for next time.
		//Note how this operation would have happened below here instead of now if addThis was positive.
		if (addToAddThis <= 0) {
			passedItem.setAddThis(passedItem.getAddThis()+addToAddThis);
		}
		...
		//Change add this after the index is processed.
		if (addToAddThis > 0) {
			passedItem.setAddThis(passedItem.getAddThis()+addToAddThis);
		}
		
		//Change modifierIdentifier if entry has modifiers.
		if (passedItem.getToggleable() == true) {
		...
		if (hasTodayEntry) {
			...
			try {
				...
				if (choice == 0) {
					...
					System.out.println();
					if (checker == 0) {
						processIndex(entries.get(index), addToAddThis);
						Collections.sort(entries, new DateComparator());
					}
					else {
	...
	public void takeQuiz() {
		...
		try {
			...
			System.out.println("Could not find " + testManager.configFilePath);
			noExceptionThrown = false;
		}
		if (noExceptionThrown) {
	...
	public void runMenu() {
		...
		//A variable to store the user's choices.
		int choice = -1;

		//Whilen user does not say to quit the program.
		while (choice != 0) {
